"""
qupo.py
QUadratic POlynomial initialized network
...
class Qupo(nn.Module):
    def __init__(self,L,N_in,N_out,a=-1,b=1):
        """
        L -- number of hidden layers
        n -- dimension of the input
        """
        super(Qupo,self).__init__()
        ...
        # first hidden layer
        self.hidden.append(nn.Linear(N_in,middlen))
        ...
        # output
        self.hidden.append(nn.Linear(middlen,N_out))
            ...
            # 2 or more case
                ...
                for j in range(0,2*n-1):
                    for i in range(0,self.N_out):
                        self.hidden[-1].weight.data[i,4*j:4*(j+1)] = torch.tensor(
                        [-2*C/(2**(2*k)),
                        4*C/(2**(2*k)),
                        -2*C/(2**(2*k)),
                        1.0],dtype = torch.float)
